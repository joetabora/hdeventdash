...
import { uploadMedia } from "@/lib/events";
import { mediaTagSchema } from "@/lib/validation/event-mutation-schemas";
...
export async function POST(
  ...
  const tagParsed = mediaTagSchema.safeParse(
    typeof tagRaw === "string" ? tagRaw : ""
  );
  if (!tagParsed.success) {
    return NextResponse.json({ error: "Invalid media tag." }, { status: 400 });
  }
  ...
  try {
    ...
    console.error("POST /api/events/[eventId]/media:", e);
